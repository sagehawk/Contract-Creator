...
function buildProposalHTML(data: ProposalData): string {
  ...
  const painPointCards = data.painPoints.map((p) => `
    <div class="bg-red-500/5 border border-red-500/10 rounded-xl p-5">
      <div class="flex items-start gap-3">
        ...
        <div><div class="font-semibold text-white text-sm">${esc(p.title)}</div><div class="text-slate-400 text-sm mt-1">${esc(p.description)}</div></div>
      </div>
    </div>`).join("\n");
  ...
  const whyUsItems = data.whyUsBullets.map((b) => `
    <div class="flex gap-4 items-start">
      ...
      <div><div class="font-semibold text-white">${esc(b.heading)}</div><div class="text-slate-400 text-sm mt-1 leading-relaxed">${esc(b.detail)}</div></div>
    </div>`).join("\n");

  const features = data.pricingBreakdown.includedFeatures.map((f) => `
    <li class="flex items-center gap-2"><svg class="w-4 h-4 text-emerald-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg><span class="text-slate-300 text-sm">${esc(f)}</span></li>`).join("\n");

  const slaCards = data.pricingBreakdown.sla.map((s) => `
    <div class="bg-white/[0.03] rounded-xl p-4 text-center border border-white/5"><div class="text-lg font-bold text-blue-300">${esc(s.value)}</div><div class="text-xs text-slate-500 mt-1">${esc(s.label)}</div></div>`).join("\n");

  const addOnRows = data.pricingBreakdown.addOns.map((a) => `
    <tr class="border-b border-white/5"><td class="py-4 pr-4"><div class="font-semibold text-white">${esc(a.name)}</div><div class="text-sm text-blue-300/80 mt-1 italic">&rarr; ${esc(a.reason)}</div></td><td class="py-4 text-right font-mono font-bold text-blue-300 whitespace-nowrap align-top">${esc(a.price)}</td></tr>`).join("\n");

  const roiCards = data.roiProjections.map((r) => `
    <div class="bg-white/[0.03] rounded-xl p-5 text-center border border-white/5"><div class="text-2xl font-black text-white">${esc(r.value)}</div><div class="text-sm text-slate-400 mt-1">${esc(r.metric)}</div></div>`).join("\n");
...
const tools: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    ...
    description: "Generates a professional HTML sales proposal and writes it to disk. Pass structured proposal data as JSON — the tool handles all HTML rendering internally using a premium Tailwind template. Do NOT pass raw HTML.",
    input_schema: {
      type: "object" as const,
      properties: {
        companyName: { type: "string" }, tagline: { type: "string" },
...
const SYSTEM_PROMPT = `You are an elite autonomous sales agent for a cutting-edge supply chain SaaS company called "ChainForge AI". Your mission is to create a highly personalized, compelling sales proposal.
...
- Structure the pricing breakdown from the getPricing data, and for each add-on, write a reason why THIS specific client needs it.
...
// ─────────────────────────────────────────────────────────────
//  SSE-Streamed ReAct Agent Loop
// ─────────────────────────────────────────────────────────────
type SSESend = (event: string, data: Record<string, unknown>) => void;

async function runAgentStreaming(clientName: string, productTier: string, send: SSESend) {
  ...
  function executeTool(name: string, input: Record<string, unknown>): string {
    switch (name) {
      ...
      case "generateProposalHTML": {
        ...
        generatedHTML = html;
        return `Proposal written to ${outPath} (${html.length} bytes)`;
      }
  ...
  const messages: Anthropic.Messages.MessageParam[] = [
    { role: "user", content: `Create a personalized sales proposal for **${clientName}** for our **${productTier}** product. Research the client thoroughly, pull our pricing details, then generate a beautiful HTML proposal document. Begin now.` },
  ];
  ...
  while (step < maxSteps) {
    ...
    for (const block of assistantContent) {
      if (block.type === "text") {
        send("reasoning", { content: block.text });
...
function serveStatic(res: ServerResponse) {
  try {
...
function handleSSE(req: IncomingMessage, res: ServerResponse) {
  ...
  const clientName = url.searchParams.get("clientName") ?? "Globex Corp";
  const productTier = url.searchParams.get("productTier") ?? "Enterprise Logistics API";
  ...
  runAgentStreaming(clientName, productTier, send)
...
server.listen(PORT, () => {
  console.log(`\n\x1b[36m\x1b[1m  ⚡ ChainForge AI — Autonomous Sales Agent\x1b[0m`);
  console.log(`\x1b[2m  Server running at \x1b[0m\x1b[1mhttp://localhost:${PORT}\x1b[0m\n`);
});
